...
func TestIfElseExpressions(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if ok {
			testIntegerObject(t, evalueated, int64(integer))
		} else {
			testNullObject(t, evalueated)
		}
	}
}
...
func TestFunctionObject(t *testing.T) {
	...
	if !ok {
		t.Fatalf("object is not function, fot=%T (%+v)", evaluated, evaluated)
	}
...
func TestArrayIndexExpresions(t *testing.T) {
	tests := []struct {
		input    string
		expected interface{}
	}{
